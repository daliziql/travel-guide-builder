...
## 2. 必备板块（按顺序）
1. Hero：标题/日期人数/时区/chips 标签（标签短，手机端会换行；不重复时间轴已表达的信息，冗余信息行直接删）。
2. 启程倒计时：T0/T1 用 `Date.UTC` 减 8h 锚定北京时间 0 点；三阶段文案（出发前/旅途中/结束后）先给用户过稿；
   只保留倒计时数字 + 一句文案，不显示「启程时刻」等附加行，文案结尾不加句号。
3. 航段总览 flightline：每程一行、严格列对齐（固定列宽，不用空格凑）；分叉回程同程多目的地、无 A/B 标签、箭头文字写「回程」；航班右侧信息（时长/里程）每行格式保持一致；前置集合段文案最简（航班号 · 集合：出发→到达（时长））。
4. 全程航图（Leaflet）：城市 pin 标签只用三字母缩写/英文（如 CAN，不混中文防溢出）、贝塞尔弧线、虚线表飞行、只保留半透明里程标注，去掉图例与线类型说明；里程用工具现算。
5. 区域总览海报：相距远的区域一块一张（制作契约见 `references/06-overview-poster.md`）；**简洁版逐日行程上方上下堆叠展示压缩 JPG**，详细版可重复出现。
6. 逐日 D1…DN：每天 = 时间轴 + 小地图双栏（窄屏堆叠），编号一一对应。
7. 住宿总览、餐饮速查、红黑榜、预算（区间+口径，合计工具算）、本地实用信息（App/值机/打车/外卖/支付/现金/小费/电话卡/装备租赁/拼团渠道/禁忌，替代旧版行前待确认）、预定专栏（团/馆/演出/餐厅一张表）、备选池、重点专栏（徒步/摄影等，时间轴放锚点 inlink 跳转；用户拍板后只留最终选择与决策表）、风险与应急（电话/使领馆/救援平台，标核验日期）。
8. 参考链接：聚合去重后收进 `.ref-mask` 弹窗，不在正文堆链接；外链一律短文字超链接 `target="_blank" rel="noopener"`，禁裸 URL。
9. 实拍弹窗 `.gal-mask`：宫格 → 大图两级。
10. 页脚只保留「个人使用，不可转载不可商用」，不写更新时间/数据来源/技术说明。
...
- 简洁版只保留：启程倒计时、hero 基本信息、**区域总览海报（压缩 JPG，多张上下堆叠、width:100% 不横滑）**、D1…DN 时间轴文字（时间+标题+副标题）与日快跳（非 D* 链接隐藏）。
